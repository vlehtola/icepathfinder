function outValue = poster_plotRouteComparisonMaskEffectKemi()
%% Ville Lehtola 2018
% POSTER: icepathfinder article plot. Kemi. multiplot with speed&time maps and ship-ship mask
% effects.
...
fprintf('AIS ship, time spent in hours: %f Top speed: %f Mean speed: %f \n', timeInHours, topSpeed, meanSpeed);
...
subaxis(1,3,3,'SpacingVert',0,'SpacingHoriz',0,'MR',0,'ML',0,'PR',0,'PL',0); 

%subplot(2,2,4);
load(fullfile(route_path, 'integratedCostMatrix.mat'), 'costMatrixAB', 'costMatrixBA');

travelTime = route(end-1,8);                  % travelTime in hours
fprintf('Travel time (optimized): %f (h) Comparison time: %f (h)\n', travelTime, (timeInHours/13.2448*topSpeed) );
A= full(costMatrixAB);
B=full(costMatrixBA);
mask=A;
mask(mask~=0)=1;
...
tc=text(ySEt+8*120+60,xSEt+20,'Time map');
tc.FontSize = 12;


%% 2nd speed map

subaxis(1,3,2,'SpacingVert',0,'SpacingHoriz',0,'MR',0,'ML',0,'PR',0,'PL',0); 
...
tc=text(ySEt+8*120+60,xSEt+20,'Speed map');
tc.FontSize = 12;
