function outValue = plotRouteComparisonMaskEffectOulu()
%% Ville Lehtola 2018
...
route_path = '/work/stormwinds/STORMWINDS/output_routeset_timesteps_noMeanfield/oulu_AISpoint-20110203/excluded_archipelago/results';
in_path = route_path;
...
manualRoute = getPlannedRoute("Oulu", latitude, longitude);
...
endVal = floor(size(xy, 1)*50/100);
%xy = xy(1:endVal, :);          % keep only half
xy = xy(endVal:end-10, :);          % keep only half

ss = shipSpeed(endVal:end-10, :);
st = shipTime(endVal:end-10, :);
...
fprintf('AIS ship, time spent in hours: %f Top speed: %f Mean speed: %f (knots)\n', timeInHours, topSpeed, meanSpeed);
...
% re-initialize
route_path = '/work/stormwinds/STORMWINDS/output_routeset_timesteps_meanField/oulu_AISpoint-20110203/excluded_archipelago/results';
in_path = route_path;
speed = 0;
...
bw = 0.03;   % bar width
set(cb3,'location','manual','position',[0.38 0.5 bw 0.4])
...
set(gcf,'color','w');                % background to white
print(h,fullfile(out_path, 'plotRouteComparisonMaskEffectOulu'),'-dpng')
