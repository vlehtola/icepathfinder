...
%% Ville Lehtola 2018
% icepathfinder article plot. Kotka. closeup
...
route_path = '/work/stormwinds/STORMWINDS/output_routeset_timesteps_meanField/kotka_AISpoint-20110301/excluded_archipelago/results';
...
manualRoute = getPlannedRoute("Kotka", latitude, longitude);
...
endVal = endVal - 1100;   % Note!! to obtain the comparison ship time
...
bw = 0.03;   % bar width
set(cb,'location','manual','position',[0.4 0.15 bw 0.4])
...
[xne, yne] = calcXY(latitude, longitude, 61, 30);
[xsw, ysw] = calcXY(latitude, longitude, 58, 19);
ylim([ xsw xne ] );
xlim([ ysw yne ] );
...
set(gcf,'color','w');                % background to white
print(h,fullfile(out_path, 'plotRouteComparisonKotkaCloseup'),'-dpng')
