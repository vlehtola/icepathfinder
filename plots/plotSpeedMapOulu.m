function outValue = plotSpeedMapOulu()
%% Ville Lehtola 2018
...
tc=text(xSEt+120*3.75, ySEt+60,'(b)');
tc.FontSize = 18;

%% ice map
subaxis(1,2,1,'SpacingVert',0.0,'SpacingHoriz',0.1);

img = imread('/home/vlehtola/Dropbox/Indoor navigation/article-icepathfinder/oulu_ice_snapshot.tiff');
imshow(img(:,:,1:3));
...
f_height=3;
f_width=3;
set(gcf,'Position',[100   200   2*round(f_width*font_rate*144)   1.15*round(f_height*font_rate*144)]);
set(gcf,'color','w');                % background to white
set(gca, 'FontName', 'Times')
print(h,fullfile(out_path, 'plotSpeedMapOulu'),'-dpng')
